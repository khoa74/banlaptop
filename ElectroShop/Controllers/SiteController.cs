...
namespace ElectroShop.Controllers
{
    public class SiteController : Controller
    {
        ...
        {          
            ...
            var _api = Url.Action("GetAll", "Category", new { httproute = "DefaultApi", Status = 1});
            ...
            if (result.IsSuccessStatusCode)
            {
                ...
                readTask.Wait();
                IEnumerable<MCategory> list = null;
        ...
        {
            MPost e = db.Posts.Where(x => x.Type == "page").First();
            var client = new HttpClient();
            var _api = Url.Action("Get", "Post", new { httproute = "DefaultApi", slug = slug });
            var _url = Request.Url.Scheme + "://" + Request.Url.Authority + _api;
            ...
            if (result.IsSuccessStatusCode)
            {
                var readTask = result.Content.ReadAsAsync<MPost>();
                readTask.Wait();

                e = readTask.Result;

            }
            return View("PostPage", e);
        }
